...
contract Auction {
    ...
    function start(address _nft, uint _nftId, uint _startingBid, uint _endBlock) external {
   ...
   function bid(uint nftTokenID) external payable {
       ...
       require(block.number < etblock, "Auction has ended. You cannot bid now");
    ...
    function cancelAuc(uint nftTokenID) public {
        ...
        require(block.number < etblock, "Auction was ended successfully. You cannot cancel now.");
        require(nftTokenID == nftId, "Wrong tokenID ");
        cancelled= true;
    ...
    function end(uint nftTokenID) external {
        require(started, "not started");
        require(block.number >= etblock, "not ended");
        require(!ended, "ended");
