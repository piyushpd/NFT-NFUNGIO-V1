...
contract Auction {
    ...
    function start(address _nft, uint _nftId, uint _startingBid, uint32 numberMins, uint32 numberHours, uint32 numberDays) external {
        ...
        started = true;
        endAt = uint32(block.timestamp) + (numberMins * 1 minutes) + (numberHours * 1 hours) + (numberDays * 1 days);
